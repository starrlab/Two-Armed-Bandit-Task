...
const KEYBOARD_PRESS_LEFT = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(49); //This is the arrow key code
const KEYBOARD_PRESS_LEFT_MIDDLE = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(50); //This is the arrow key code
const KEYBOARD_PRESS_RIGHT_MIDDLE = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(51); //This is the arrow key code
const KEYBOARD_PRESS_RIGHT = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(52); //This is the arrow key code
...
csvData += "KEYBOARD_PRESS_LEFT_MIDDLE," + KEYBOARD_PRESS_LEFT_MIDDLE + "\n";
csvData += "KEYBOARD_PRESS_RIGHT_MIDDLE," + KEYBOARD_PRESS_RIGHT_MIDDLE + "\n";
...
let action = {
    type: "html-keyboard-response",
    choices: [KEYBOARD_PRESS_LEFT, KEYBOARD_PRESS_LEFT_MIDDLE, KEYBOARD_PRESS_RIGHT_MIDDLE, KEYBOARD_PRESS_RIGHT],
    prompt: function() {
        return "<div><h1>" + formatter.format(rewardCount).toString() + "</h1></div>"
    },
    stimulus:     "<div  '><h1>Pull</h1></div>" +
    "<div class='container'>"+
    "<div  '><img src='../images/HandleLeft.png'></img></div>" +
    "<div  '><img src='../images/HandleLeft.png'></img></div>" +
    "<div  '><img src='../images/HandleRight.png'></img></div>" +
    ...
    on_finish: function (data) {
        ...
        if(userResponseKeyPress == KEYBOARD_PRESS_LEFT){
            userRewardForCurrentTrial = LEFT_ARM_REWARDS[((currentBlockNumber-1)*NUMBER_OF_TRIALS+currentTrialNumber)-1];
        }
        else if(userResponseKeyPress == KEYBOARD_PRESS_LEFT_MIDDLE){
            userRewardForCurrentTrial = LEFT_MIDDLE_ARM_REWARDS[((currentBlockNumber-1)*NUMBER_OF_TRIALS+currentTrialNumber)-1];
        }
        else if(userResponseKeyPress == KEYBOARD_PRESS_RIGHT_MIDDLE){
            userRewardForCurrentTrial = RIGHT_MIDDLE_ARM_REWARDS[((currentBlockNumber-1)*NUMBER_OF_TRIALS+currentTrialNumber)-1];
        }
        else if(userResponseKeyPress == KEYBOARD_PRESS_RIGHT){
...
let feedback = {
    ...
    stimulus: function() {
        if(userResponseKeyPress == KEYBOARD_PRESS_LEFT)
        {
            return "<div  '><h1>You won " + formatter.format(userRewardForCurrentTrial).toString() + "</h1></div>" +
            "<div class='container'>"+
